...
class LBIBForm:
    def __init__(
        ...
        self.pressure=ti.field(float, shape=(NX, NY)) 
        ...
        self.rho_gas=0.15
        self.rho0=1.0
        self.rho_solid=0.15+0.1*(1.95-0.15)
    ...
    def hydroInfo(self,omega:float):
        self.omega=omega
    ...
    def collide_bulk(self):
        ...
        for m in range(count_FluidGroup):
            ...
            CollisionOperator =-self.omega*(self.f[i,j]-feqeq)
            ForceTerm=(1.0-0.5*self.omega)*force_ij

            self.f2[i,j]=self.f[i,j]+CollisionOperator+ForceTerm
