...
pos_np = np.array([[100.0,100.0]]).astype(np.float32)
...
radius_np = np.array([15.0]).astype(np.float32)
...
test.init_SC(epslion=1.0,k1=0.15)
#==============================================LBM boundary
...
for i in range (10):
    ...
    vel_img = cm.plasma(test.post_vel()/0.5)
    rho_img=cm.coolwarm(test.pos_density())
    
    img = np.concatenate((vel_img,rho_img), axis=1)
